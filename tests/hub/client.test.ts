...
describe("HubClient", () => {
  ...
  it("sendMessage 发送正确的请求", async () => {
    ...
    const result = await client.sendMessage({ userId: "user-001", text: "test", traceId: "t1" });
  ...
  it("业务错误时抛出异常", async () => {
    ...
    await expect(client.sendMessage({ userId: "u1", text: "hi" })).rejects.toThrow("用户不存在");
  });

  it("registerTools 发送 PUT 请求", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({ ok: true, headers: new Headers() });
    await client.registerTools([{ name: "convert", description: "换算", command: "convert" }]);
    expect(vi.mocked(globalThis.fetch)).toHaveBeenCalledWith(

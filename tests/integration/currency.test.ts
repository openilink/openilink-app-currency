...
describe("汇率换算集成测试", () => {
  ...
  it("challenge 握手", async () => {
    const event = {
      v: "1", type: "challenge", challenge: "test_123",
      trace_id: "t1", installation_id: INSTALLATION_ID, bot: { id: "b1" },
    };
    ...
    expect(res.ok).toBe(true);
    expect((await res.json() as any).challenge).toBe("test_123");
  });

  it("无效签名被拒绝（401）", async () => {
    const event = {
      v: "1", type: "event", trace_id: "t1", installation_id: INSTALLATION_ID, bot: { id: "b1" },
      event: { type: "command", id: "e1", timestamp: new Date().toISOString(), data: { command: "convert_currency" } },
    };
    const res = await originalFetch(`http://localhost:${APP_PORT}/hub/webhook`, {
      method: "POST",

...
async function requestHandler(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (pathname === "/manifest.json" && req.method === "GET") {
      ...
      res.end(JSON.stringify({ ...manifest, tools: definitions }, null, 2));
      return;
    }
...
function shutdown(signal: string): void {
  ...
  store.close();
  server.close(() => { console.log("[app] 服务已关闭"); process.exit(0); });
  setTimeout(() => { console.error("[app] 关闭超时，强制退出"); process.exit(1); }, 10_000);
}
